...
```
We only keep genes with log2FC value greater than 1 and p_val lesser than 1%. We end up with 2063 genes. If we want to have less genes, we can always add more constraints, or keep the n genes with higher log2FC

```{r}
umap.markers[umap.markers$avg_log2FC > 1 & umap.markers$p_val < 0.01, ]
```
